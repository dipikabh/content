...
If _max_ < _min_, then _max_ is ignored and `minmax(min,max)` is treated as _min_. As a maximum, a {{cssxref("flex","&lt;flex&gt;")}} value sets the flex factor of a grid track; it is invalid as a minimum.
