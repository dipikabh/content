...
{{HTMLElement("input")}} elements of type **`color`** provide a user interface element that lets a user specify a color, either by using a visual color picker interface or by entering the color into a text field in a [CSS color value](/en-US/docs/Web/CSS/Reference/Values/color) format.
...
A [CSS color value](/en-US/docs/Web/CSS/Reference/Values/color).
...
- `colorspace` {{experimental_inline}}
  - : Defines the {{glossary("color space")}} for the color and hints at the desired user interface for the color picker widget. Possible {{Glossary("enumerated")}} values are:
    - `"limited-srgb"`: The color is in the {{glossary("RGB", "sRGB")}} color space. This includes [`rgb()`](/en-US/docs/Web/CSS/Reference/Values/color/rgb), [`hsl()`](/en-US/docs/Web/CSS/Reference/Values/color/hsl), [`hwb()`](/en-US/docs/Web/CSS/Reference/Values/color/hwb), and {{cssxref("hex-color")}} values. The color value is limited to 8-bits per `r`, `g`, and `b` component. This is the default.
